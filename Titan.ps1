<#
.SYNOPSIS
    Titan Optimizer (Bootstrapper & Launcher)
...
if ($IsWebExecution -or -not $HasSrc) {
    Write-Host " [BOOTSTRAPPER] Initializing Titan Optimizer..." -ForegroundColor Cyan
    
    $InstallDir = "$env:TEMP\TitanOptimizer"
    $ZipPath = "$env:TEMP\TitanOptimizer_Repo.zip"
    $ExtractPath = "$env:TEMP\TitanOptimizer_Extract"
    ...
    try {
        ...
        $LauncherScript = Join-Path $InstallDir "Titan.ps1"
...
try {
    ...
    Write-Host " [LOADER] Loading Titan Optimizer..." -ForegroundColor Cyan
    ...
    Initialize-Logger -LogPath (Join-Path ([Environment]::GetFolderPath("Desktop")) "TitanOptimizer.log")
